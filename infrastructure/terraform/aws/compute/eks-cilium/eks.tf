...
module "aws_eks" {
  ...
  cluster_name    = "${var.workspace}-${var.env}-${var.resource_name}"
  ...
  tags = local.common_tags
   map_users = concat([
    {
      userarn  = "arn:aws:iam::${data.aws_caller_identity.this.account_id}:user/${var.workspace}-aws-eks-circleci"
      username = "${var.workspace}-aws-eks-circleci"
      groups   = ["system:masters"]
    }    
  ], local.auth_users)
}
